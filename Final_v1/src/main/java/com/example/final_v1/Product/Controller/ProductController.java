...
public class ProductController {
    ...
    @GetMapping(path = {"/page/{pageNo}", "/page"})
    ResponseObj getAllProduct(@PathVariable(required = false, value = "pageNo") Integer pageNo) {
        int pageSize = 20;
        if(pageNo != null) {
            return new ResponseObj("OK", "Get all products", productService.findPaginate(pageNo, pageSize));
        }
        return new ResponseObj("OK", "Get all products", productService.findPaginate(1, pageSize));
    }
    ...
    ResponseObj deleteProduct(@PathVariable long id) {
        ...
        if (check == 1) {
            return new ResponseObj("OK", "Delete successfully", id);
        }
